 ...
 - 링크 계층
   ...
   - 이더넷 프레임
     ...
     - DMAC, SMAC
       - 수신, 송신 MAC 주소를 말합니다.
     - EtherType
       - 데이터 계층 위의 계층인 IP 프로토콜을 정의합니다. 예를 들면 IPv4 or IPv6가 됩니다.
     - Payload
       - 전달받은 데이터
     - CRC
       - 에러 확인 비트
###### MAC 주소 - 네트워크에 연결하기 위한 장치(LAN카드)가 있는데, 이를 구별하기 위한 식별번호를 말한다.
***
 - 계층 간 데이터 송수신 과정
   ...
   - 캡슐화 과정
     ...
     - 애플리케이션 계층 : 데이터
       - 애플리케이션 계층의 데이터가 전송 계층으로 절달 되면서 '세그먼트' 또는 '데이터 그램'화 되면 TCP(L4)헤더가 붙게 됩니다.
     - 전송 계층 : 데이터 + TCP(L4)헤더
       - 인터넷 계층으로 가면서 IP(L3)헤더가 붙여지게 되며 '패킷'화가 됩니다.
     - 인터넷 계층 : 데이터 + TCP(L4)헤더 + IP(L3)헤더
       - 링크 계층으로 가면서 프레임 헤더와 프레임 트레일러가 붙어 프레임화가 됩니다.
     - 링크 계층 : 프레임 트레일러 + 데이터 + TCP(L4)헤더 + IP(L3)헤더 + 프레임헤더
   - 비캡슐화 과정
     - 비캡슐화 과정은 하위 계층에서 상위 계층으로 가며 각 계층의 헤더 부분을 제거하는 과정을 말합니다.
     - 캡슐화된 데이터를 받게 되면 링크 계층에서 부터 타고 올라가면서 프레임화된 데이터는 다시 패킷화를 거쳐 세그먼트, 데이터그램화를 거쳐 메시지화가 되는 비캡슐화 과정이 일어납니다.
     - 마지막으로 사용자에게 애플리케이션의 PDU인 메시지로 전달 됩니다.
 - PDU(Protocol Data Unit)
   - 네트워크의 어떠한 계층에서 계층으로 데이터가 전달될 때 한 덩어리의 단위를 PDU라고 합니다.
   - PDU는 제어관련 정보들이 포함된 '헤더', 데이터를 의미하는 '페이로드'로 구성되어 있습니다. 계층마다 부르는 명칭이 다릅니다.
     - 애플리케이션 계층 : 메시지
     - 전송 계층 : 세그먼트(TCP), 데이터그램(UDP)
     - 인터넷 계층 : 패킷
     - 링크 계층 : 프레임(데이터 링크 계층), 비트(물리계층)
   - PDU 중 아래 계층인 비트로 송수신하는 것이 모든 PDU 중 가장 빠르고 효율성이 높습니다.
   - 애플리케이션 계층에서는 문자열을 기반으로 송수신을 하는데 그 이유는 헤더에 authorization 값 등 다른 값들을 넣는 확장이 쉽기 때문입니다.